...
console.log('🚀 Setting up Lots Database System...\n');
...
async function setupDatabase() {
  try {
    ...
    try {
      ...
      console.log('⚠️  Migration failed, trying to create tables manually...');
      
      // If migrations fail, try to create tables manually
      const db = require('knex')(knex);
      
      // Create tables manually
      await db.schema.createTableIfNotExists('lot_categories', (table) => {
      ...
      await db.schema.createTableIfNotExists('lot_tags', (table) => {
        table.increments('id').primary();
        table.string('name', 100).notNullable().unique();
        table.string('description', 500);
      ...
      await db.schema.createTableIfNotExists('lots', (table) => {
        table.increments('id').primary();
        ...
        table.string('additional_message_id', 50);
        table.jsonb('metadata').defaultTo('{}');
        table.timestamps(true, true);
      });
      
      await db.schema.createTableIfNotExists('lot_photos', (table) => {
        table.increments('id').primary();
      ...
      await db.schema.createTableIfNotExists('lot_participants', (table) => {
        table.integer('lot_id').unsigned().notNullable().references('id').inTable('lots').onDelete('CASCADE');
      ...
      await db.schema.createTableIfNotExists('lot_tag_assignments', (table) => {
        table.integer('lot_id').unsigned().notNullable().references('id').inTable('lots').onDelete('CASCADE');
      ...
      await db.schema.createTableIfNotExists('user_preferences', (table) => {
        ...
        table.jsonb('favorite_tags').defaultTo('[]');
        ...
        table.decimal('max_price_preference', 10, 2);
        table.timestamps(true, true);
      });
      
      await db.schema.createTableIfNotExists('user_favorites', (table) => {
        table.bigInteger('user_id').notNullable();
        table.integer('lot_id').unsigned().notNullable().references('id').inTable('lots').onDelete('CASCADE');
        table.timestamp('favorited_at').defaultTo(db.fn.now());
    ...
    try {
      ...
      console.log('⚠️  Seeding failed, creating seed data manually...');
      
      // If seeding fails, create seed data manually
      const db = require('knex')(knex);
      
      // Insert categories
      await db('lot_categories').insert([
        { id: 1, name: 'Миниатюры', description: 'Фигурки и миниатюры для настольных игр', icon: '🎲', is_active: true },
        { id: 2, name: 'Террейн', description: 'Ландшафт и окружение для игр', icon: '🏔️', is_active: true },
        { id: 3, name: 'Краски и материалы', description: 'Материалы для покраски и сборки', icon: '🎨', is_active: true },
        { id: 4, name: 'Книги и правила', description: 'Игровые книги, правила и литература', icon: '📚', is_active: true },
        { id: 5, name: 'Аксессуары', description: 'Дополнительные игровые аксессуары', icon: '🎯', is_active: true },
        { id: 6, name: 'Электроника', description: 'Электронные компоненты и устройства', icon: '⚡', is_active: true },
        { id: 7, name: 'Другое', description: 'Прочие категории', icon: '📦', is_active: true }
      ]);
      
      // Insert hashtag-style tags
      await db('lot_tags').insert([
      ...
      console.log('✅ Seed data created manually');
      await db.destroy();
    }
    
    console.log('\n🎉 Lots Database System setup completed successfully!');
    console.log('\n📋 What was created:');
    console.log('   • Database tables for lots, categories, tags, and user preferences');
    console.log('   • 7 main categories with 84 hashtag-style tags');
    console.log('   • User preferences and favorites system');
    console.log('   • Search and filtering capabilities');
    console.log('   • Hashtag-style tags for easy discovery and search');
    
    console.log('\n🏷️ Popular hashtags you can use:');
    console.log('   • #warhammer40k, #dnd, #starwars, #marvel');
    console.log('   • #acrylic, #fantasy, #scifi, #kickstarter');
    console.log('   • #limited, #custom, #handmade, #vintage');
    
    console.log('\n🚀 Next steps:');
    console.log('   1. Restart your bot to load the new lots system');
    console.log('   2. Use /preferences to set up your user preferences');
    console.log('   3. Use /search to explore lots and hashtags');
    console.log('   4. Create your first lot with "гоблин, хочу создать лот"');
    console.log('   5. Try searching with hashtags: /search #warhammer40k');
