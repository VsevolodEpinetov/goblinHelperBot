...
module.exports = Composer.action('actionAdditional', async ctx => {
  try {
    ...
    await ctx.telegram.sendMessage(telegramID, `Твоя оплата на месяц "${ctx.globalSession.currentMonth}" подтверждена, включая плюс.\n\nСсылка для вступления в основную группу: ${ctx.globalSession.months[ctx.globalSession.currentMonth].mainInvitationLink}\nСсылка для вступления в плюсовую группу: ${ctx.globalSession.months[ctx.globalSession.currentMonth].plusInvitationLink}.\n\nБот подтвердит твоё вступление`)
