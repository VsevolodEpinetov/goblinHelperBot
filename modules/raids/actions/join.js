...
// Helper function to update public raid message
...
module.exports = async (ctx) => {
  try {
    console.log('🔍 Join action called with context:', {
    ...
    if (!ctx.match || !ctx.match[1]) {
      console.log('❌ No match data found for join action');
      return;
    }
    
    const raidId = parseInt(ctx.match[1]);
    console.log(`🔍 Attempting to join raid ${raidId} for user ${ctx.from.id}`);
    ...
    // Try to join the raid
    const result = await raidsService.joinRaid(raidId, userData);
    ...
    await ctx.answerCbQuery(`✅ Вы присоединились к рейду! Теперь с человека: ${pricePerPerson} ${raid.currency}`);
    ...
    if (ctx.callbackQuery) {
      await ctx.answerCbQuery('❌ Произошла ошибка при присоединении к рейду', { show_alert: true });
    }
  }
};
