...
describe('promo.service.formatTimeRemaining', () => {
  ...
  it('formats hours and minutes', () => {
    const future = new Date(now.getTime() + (2 * 60 + 30) * 60_000);
    expect(formatTimeRemaining(future, now)).toBe('2ч 30м');
  });

  it('formats minutes only when under an hour', () => {
    const future = new Date(now.getTime() + 45 * 60_000);
    expect(formatTimeRemaining(future, now)).toBe('45м');
  });

  it('returns "0 минут" when already past', () => {
    const past = new Date(now.getTime() - 1000);
    expect(formatTimeRemaining(past, now)).toBe('0 минут');
  });
});
