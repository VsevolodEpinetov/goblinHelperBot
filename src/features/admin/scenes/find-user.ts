...
findUserScene.enter(async (ctx) => {
  await ctx.reply('🔍 Кого выследить? Дай id или @username. Отмена — /cancel.');
});

findUserScene.command('cancel', async (ctx) => {
  ctx.scene.state = {};
  await ctx.scene.leave();
  await ctx.reply('🕯 Отменил, как велишь.', backToHubKeyboard());
});
...
findUserScene.on('text', async (ctx) => {
  try {
    ...
    if (rows.length === 0) {
      await ctx.reply('👁‍🗨 Такого следа в книгах нет. Попробуй иначе — или /cancel.');
      return;
    }
